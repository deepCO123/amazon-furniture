...
import { ArrowRight, Sparkles, Star, Truck, Shield, Mouse, Wrench } from "lucide-react";
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
              ...
              <h1 className="text-3xl sm:text-5xl lg:text-6xl font-extrabold text-white leading-[1.2] mb-4">
                {isEn ? (
                  titleWords.map((word, wi) => (
                    <span key={wi} className="inline-block me-[0.3em]">
                  ...
                  <span className="block leading-snug">
                    ...
                    <span className="hero-gradient-text block">{t.heroTitle2}</span>
                  </span>
                )}
              </h1>
              ...
              <motion.p
                ...
                className="text-white/70 text-lg sm:text-xl mb-10 leading-relaxed max-w-lg"
              >
                {t.heroSubtitle}
              </motion.p>
              ...
              >
                <Link href="/products">
                  <Button variant="secondary" size="lg" className="group font-bold">
                    {t.shopNow}
                    <ArrowRight size={18} className="ms-2 group-hover:translate-x-1 transition-transform" />
                  </Button>
                </Link>
                <Link href="/about">
                  <Button variant="outline" size="lg" className="border-white/20 text-white hover:bg-white hover:text-primary font-bold">
                    {t.exploreCollection}
                  </Button>
                </Link>
              </motion.div>
              ...
              >
                {[
                  { icon: Truck, text: t.freeShippingBadge },
                  { icon: Shield, text: t.featWarranty },
                  { icon: Star, text: t.ratingText },
                ].map(({ icon: Icon, text }) => (
              ...
              >
                {[
                  { value: 500, suffix: "+", label: t.allProducts },
                  { value: 10, suffix: "k+", label: isEn ? "Happy Customers" : (language === "sa" ? "عميل راضٍ" : "عميل سعيد") },
                  { value: 30, suffix: "+", label: isEn ? "Years Experience (1994)" : "عاماً من الخبرة (1994)" },
                ].map((stat) => (
                  <div key={stat.label} className="relative">
                    ...
                    <div className="absolute -bottom-2 start-0 w-8 h-0.5 bg-accent/30 rounded-full" />
                  </div>
                ))}
              </motion.div>
            </div>
            ...
            >
              ...
              {/* Floating assembly card */}
              {!prefersReduced && (
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: 1.5, duration: 0.6 }}
                  className="absolute right-0 sm:right-4 bottom-4 sm:bottom-8 bg-white/95 backdrop-blur-sm rounded-xl p-3 shadow-xl z-40 border border-emerald-100"
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <p className="text-xs font-bold text-primary">
                        {isEn ? "Free Assembly" : "التركيب مجاناً ✓"}
                      </p>
                      <p className="text-[10px] text-muted">
                        {isEn ? "Professional setup" : "معاينة وتركيب احترافي"}
                      </p>
                    </div>
                  </div>
                </motion.div>
          ...
          <div className="flex animate-marquee whitespace-nowrap py-3">
            {[...Array(2)].map((_, i) => (
              <div key={i} className="flex items-center gap-8 mx-8">
                {[
                  isEn ? "Free Professional Assembly ✓" : "التركيب مجاناً بالكامل ✓",
                  isEn ? "Handcrafted Quality" : "جودة تصنيع فاخرة",
                  isEn ? "Natural Solid Wood" : "خامات خشب طبيعي مستدامة",
                  isEn ? "Comprehensive Warranty" : "ضمان شامل للجودة",
                  isEn ? "Direct WhatsApp Ordering" : "تأكيد فوري عبر واتساب",
                  isEn ? "Custom Sizing & Orders" : "تفصيل وتخصيص حسب الطلب",
                ].map((text, j) => (
